...
export function MessageWithLink({ message, buttonText, link, onClick }: { buttonText: string; link?: string; message?: string; onClick?: () => void }) {
  return (
    <div className="flex flex-col items-center gap-4 text-pretty text-xl font-bold tablet:text-2xl">
      <span>{message}</span>
      {link ? (
        <Link href={link} className="flex items-center rounded-md bg-black px-4 py-2 text-xs text-white mobile:text-sm tablet:text-base">
          {buttonText}
          <Image src="/icons/arrow-right.svg" alt="오류 화살표" width={24} height={24} className="invert" />
        </Link>
      ) : (
        <button type="button" onClick={onClick} className="flex items-center rounded-md bg-black px-4 py-2 text-xs text-white tablet:text-sm">
          {buttonText}
          <Image src="/icons/arrow-right.svg" alt="오류 화살표" width={24} height={24} className="invert" />
        </button>
      )}
